...
# TAKE_ONE = (0, good_index)
# TAKE_CAMELS = (1,)
# SELL = (2, good_index, count)
# EXCHANGE = (3, take_tuple, give_goods_tuple, give_camels)
...
@dataclass
class PlayerState:
    hand: list[int] = field(default_factory=lambda: [0] * _N_GOODS)  # count per good
    camels: int = 0
    score: int = 0
...
@dataclass
class GameState:
    # Market as counts: [good0, good1, ..., good5, camels]
    market: list[int] = field(default_factory=lambda: [0] * (_N_GOODS + 1))
    deck: list[int] = field(default_factory=list)  # list of good indices (6=camel)
    players: tuple[PlayerState, PlayerState] = field(
        default_factory=lambda: (PlayerState(), PlayerState())
    )
    tokens: list[list[int]] = field(default_factory=list)  # per good type
    ...
    def new_round(rng: random.Random | None = None) -> GameState:
        ...
        # Build deck as list of indices (0-5 = goods, 6 = camel)
        deck: list[int] = []
        for g in GOODS:
    ...
    def get_legal_actions(self) -> list[tuple]:
        ...
        for i in range(_N_GOODS):
            ...
            if count == 0:
                continue
            if i < 3:  # precious (diamond, gold, silver)
                if count >= 3:
        ...
        # Exchange: simplified — take each single market good type,
        # give back one from hand or a camel. Plus take-2 combos.
    ...
    def _add_exchange_actions(self, actions: list[tuple]) -> None:
        ...
        # Collect what we can give (hand goods + camels)
        ...
        # Size-2 exchanges: take 2 goods from market, give 2 back
        # Generate pairs of market goods to take
        for ti in range(len(market_goods)):
            ...
            for tj in range(ti, len(market_goods)):
                ...
                # Option 3: give 2 goods (not in take)
                gave = []
                remaining = 2
                for gk, gk_c in giveable:
                    if gk in take:
        ...
        # Size-3 exchanges if we have enough to give
        if total_giveable >= 3 and len(market_goods) >= 2:
            # Take top 3 market goods by "value" (precious first)
            all_market = []
            for i, c in market_goods:
                all_market.extend([i] * min(c, 3))
            all_market.sort()  # precious goods have lower indices

            if len(all_market) >= 3:
                take = tuple(all_market[:3])
                # Give: camels first, then hand goods
                give_goods = []
    ...
    def apply_action(self, action: tuple) -> GameState:
        ...
        if atype == ACT_TAKE_ONE:
            ...
            # Remove from hand/herd, add to market
